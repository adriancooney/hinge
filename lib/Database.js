 ...
 * @return {this}      Database.
 */
Database.prototype.into = function(tables) {
	if(!this.queryString) throw new Error("Please create a new query using Database#insert or Database#update etc.");
	if(!tables) throw new Error("Please provide a table to insert into in Database#into or Database#from");

	this.queryString = this._replace(this.queryString, "table", (tables instanceof Array) ? tables.join(", ") : tables);
